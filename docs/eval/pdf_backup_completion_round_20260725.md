...
## 10. 第四步：为 40 个缺口逐条评估“权威替代源”（2026-07-26）

针对“能否换新的权威来源代替这些取不到原件的数据”这一问题，按“只接受同一份文档的其他权威落点或官方新版、拒绝非权威转载冒充出处”的原则逐条评估。合法替换的既有范式见 `artifacts/pdf_source_backups_20260725/pdf_url_replacements_20260725.csv` 前 3 行（Yale 当前下载路径、Guelph 2025 新版、Agilent 当前文件名，均为同机构/同发布方权威新落点、已下载校验）。本步新增 5 条**已否决候选**记录在同一文件，结论：

| 缺口（代表） | 评估的替代候选 | 是否同一份权威文档 | 判定 |
|---|---|---|---|
| OSHA3404 等 30 个 osha.gov 出版物/指令（68 片段） | wright.edu / wisconsin.edu 批注版、pdf4pro/readkong/scribd 聚合站 | 否，均非 OSHA 官方发布 | 拒绝（非权威转载）；govinfo/GPO 未收录 |
| NIH_Guidelines（5 片段） | Federal Register 2019-08462 | 否，是修订“最终通告”而非 132 页合订本正文 | 拒绝（官方但非同一文档） |
| Thermo Vanquish 手册（10 片段） | `knowledge1.thermofisher.com` 知识库“Save as PDF”导出 | 否，实测仅 1 页 51KB 落地页，非手册正文 | 拒绝（同发布方但非同一文档） |
| NRC ML 系列 10 个（16 片段） | ADAMS 公共检索 | 否，回跳被封路径，无稳定公开直链 | 拒绝（无可下载权威落点） |
| Berkeley 2 个（4 片段） | —（官方已网页化） | 已无 PDF 原件存在 | 拒绝（应向 EH&S 索取或按网页归档） |

**结论**：40 个缺口**没有一条**存在“可下载的、同一份权威文档”的替代落点；能搜到的替代物要么是非权威转载（政策禁止用作正式出处），要么是内容不对应的官方相邻文档，或仅是知识库落地页。因此**不改动 KB 任何一条 `source_url`**（避免用非权威源冒充出处、造成倒填/伪造出处），覆盖数维持 111/151。这次评估本身作为“已尽力寻找替代源并逐条说明为何不可用”的可追溯记录留存于 replacements CSV。
