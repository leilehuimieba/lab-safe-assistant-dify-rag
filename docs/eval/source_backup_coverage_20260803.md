# 知识库来源链接与本地备份覆盖审计（2026-08-03）

- 生成时间（UTC）：`2026-08-03T15:45:12.382931+00:00`
- 知识片段：**3009** 条；唯一来源链接：**717** 个
- 本次实测可打开：**351** 个；被站点/反爬拦截：**333** 个；网络错误：**31** 个；异常 HTTP 状态：**0** 个；未审计：**2** 个；其他状态：**0** 个（六项之和等于唯一来源链接总数）
