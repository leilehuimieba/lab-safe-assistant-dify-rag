...
def _build_emergency_rule_answer(rule_id: str, response: str, citations: list[Citation]) -> str:
    ...
    if rule_id == "R-030":
        return (
            ...
            f"{response}\n\n"
            "立即处理:\n"
            "1. 先看环境再靠近：确认没有带电设备、不明气体或气味、化学品泄漏、烟火和缺氧风险；无法确认安全时不要进入，先在门外呼救并报警说明可能涉及的化学品。\n"
            "2. 大声呼救，指定具体的人拨打急救电话并去取 AED，同时通知实验室负责人。\n"
            "3. 检查反应和呼吸：无反应且无正常呼吸时，由受训人员立即开始心肺复苏并尽快接上 AED，坚持到急救人员接手或伤者恢复自主呼吸。\n"
            "4. 有呼吸时让其平躺、双腿抬高约 30 cm，松开领口和过紧衣物，头偏向一侧防止呕吐物误吸；确认安全后可将其转移到通风良好处。\n"
            "5. 检查跌倒造成的头颈部外伤；有意识改变或颈部疼痛时固定颈部、不要移动伤者，等待急救。\n"
            "6. 即使很快清醒也必须送医评估，并记录发生经过、可能诱因和接触过的化学品（携带 SDS）。\n\n"
            "禁止事项:\n"
            "- 禁止在未确认现场安全时贸然冲入救人，避免自己成为第二名伤者。\n"
            "- 禁止给意识不清者喂水、喂药或催吐。\n"
            "- 禁止晕倒后立刻把人扶起、拍打或摇晃；疑似头颈外伤时禁止随意搬动。\n"
            "- 禁止因为伤者看起来恢复了就取消就医和事故报告。\n\n"
            "应急升级:\n"
            "- 多人同时昏倒或倒下时，优先按一氧化碳、缺氧或化学品泄漏处理：全员立即撤离、开门通风并报警，任何人不得单独进入。\n"
            "- 疑似触电时先切断电源再接触伤者（详见电气事故专项处置）；疑似吸入有害气体时在确认安全后转移到新鲜空气处。\n\n"
            "参考依据:\n"
            f"{format_citation_lines(citations)}"
        )
    if rule_id == "R-031":
        return (
            ...
            f"{response}\n\n"
            "立即处理:\n"
            "1. 立即停止取用、转注和充装，让所有人离开罐体附近，尤其不要停留在安全阀、爆破片的泄放方向和罐体正上方。\n"
            "2. 打开房门和通风，按低温液体大量汽化导致缺氧的风险处理；密闭或地下房间内的人员一律先撤到室外。\n"
            "3. 在安全距离外观察泄压装置是否被冰霜堵塞，并立即通知设备管理人员、EHS 和供气/维保单位，由受训人员按厂家程序缓慢泄压。\n"
            "4. 在现场设置警戒并记录压力读数、结霜/变形情况和发生时间，供后续检修和事故分析使用。\n\n"
            "禁止事项:\n"
            "- 禁止堵塞、锁死、压住或人为关闭安全阀和爆破片。\n"
            "- 禁止敲击、加热、倾倒或继续充装已结霜、鼓包、变形的罐体。\n"
            "- 禁止在未确认氧含量的密闭空间内滞留，禁止单人进入排查。\n"
            "- 禁止徒手接触结霜管路和阀门，避免低温冻伤。\n\n"
            "应急升级:\n"
            "- 罐体明显变形鼓包、安全阀持续大量喷放或已发生泄漏时，立即全员撤离到室外集合点并报警，等待专业人员处置。\n"
            "- 现场人员出现头晕、胸闷、口唇发紫等缺氧表现时，按人员失去反应的医疗应急同步处置。\n\n"
            "参考依据:\n"
            f"{format_citation_lines(citations)}"
        )
    return (
